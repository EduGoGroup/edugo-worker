...
func TestGetAPIIdentityConfigWithDefaults_ConValoresConfigurados(t *testing.T) {
	cfg := &Config{
		APIIdentity: APIIdentityConfig{
			BaseURL:      "http://api-identity:8070/api",
	...
	result := cfg.GetAPIIdentityConfigWithDefaults()

	assert.Equal(t, "http://api-identity:8070/api", result.BaseURL)
...
func TestGetAPIIdentityConfigWithDefaults_SinConfiguracion(t *testing.T) {
	cfg := &Config{}

	result := cfg.GetAPIIdentityConfigWithDefaults()

	assert.Equal(t, "http://localhost:8070/api", result.BaseURL, "Debería usar BaseURL por defecto")
