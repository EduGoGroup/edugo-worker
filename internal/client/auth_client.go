...
// AuthClient permite validar tokens JWT contra api-identity como autoridad central.
...
// TokenInfo contiene la información de un token validado por api-identity
...
type AuthClientConfig struct {
	BaseURL        string               // URL base de api-identity (ej: http://localhost:8081)
...
// AuthClient cliente para validar tokens con api-identity
...
// ValidateToken valida un token JWT con api-identity
...
func (c *AuthClient) ValidateToken(ctx context.Context, token string) (*TokenInfo, error) {
	...
	// 2. Llamar a api-identity con circuit breaker
...
// doValidateToken realiza la llamada HTTP a api-identity /v1/auth/verify
...
// doValidateTokensBulk realiza validación bulk con api-identity /v1/auth/verify-bulk
